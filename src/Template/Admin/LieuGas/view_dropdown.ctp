...
<?= $this->Html->link(__('Edit Lieu Gas'), ['action' => 'edit', $lieuGas->id], ['class' => 'nav-link']) ?>
<?= $this->Form->postLink( __('Delete Lieu Gas'), ['action' => 'delete', $lieuGas->id], ['confirm' => __('Are you sure you want to delete # {0}?', $lieuGas->id), 'class' => 'nav-link'] ) ?>
...
<div class="krajRegions view large-9 medium-8 columns content">
    <h3><?= h($lieuGas->lieu) ?></h3>
    <div class="table-responsive">
        <table class="table table-striped">
            
            <tr>
                <th scope="row"><?= __('Lieu') ?></th>
                <td><?= h($lieuGas->lieu) ?></td>
            </tr>
            <tr>
                <th scope="row"><?= __('Id') ?></th>
                <td><?= $this->Number->format($lieuGas->id) ?></td>
            </tr>
        </table>
    </div>
    <div class="related">
        <h4><?= __('Related Nom Gas') ?></h4>
        <?php if (!empty($lieuGas->nom_gas)): ?>
        <div class="table-responsive">
            <table class="table table-striped">
                <tr>
                ...
                <?php foreach ($lieuGas->nom_gas as $nomGas): ?>
                <tr>
                    <td><?= h($nomGas->id) ?></td>
    ...
    <div class="related">
        ...
        <?php if (!empty($lieuGas->okres_counties)): ?>
        <div class="table-responsive">
            <table class="table table-striped">
                <tr>
                    <th scope="col"><?= __('Id') ?></th>
                ...
                <?php foreach ($lieuGas->okres_counties as $prixGas): ?>
                <tr>
                    <td><?= h($prixGas->id) ?></td>
